...
for (const action of campaignActionsV2_1.campaignActionsV2) {
    ...
    const sourceTemplate = campaignActionsV2_1.campaignActionTemplates.find((template) => template.id === action.id);
    assert(sourceTemplate, `Missing source template for ${action.id}`);
    assert(JSON.stringify(action.tags) === JSON.stringify(sourceTemplate.tags), `Semantic tags were not preserved for ${action.id}`);
    assert(action.tags !== sourceTemplate.tags, `Semantic tags must be copied for ${action.id}`);
...
assert((0, actionEngine_1.getCampaignTimingMultipliers)("gotvOperation", 20, 21).turnout >
    (0, actionEngine_1.getCampaignTimingMultipliers)("gotvOperation", 3, 21).turnout, "GOTV should be stronger late");
assert((0, actionEngine_1.getCampaignTimingMultipliers)("fabricatedScandal", 20, 21).risk >
    (0, actionEngine_1.getCampaignTimingMultipliers)("fabricatedScandal", 3, 21).risk, "fabricated scandal should be riskier late");
...
assert((cleanState.partyRuntime.player.organization.ustecky ?? 0) >
    cleanBeforeOrganization, "Clean action should increase regional organization");
assert(cleanState.partyRuntime.player.reputation.authenticity >
    cleanBeforeAuthenticity, "Clean action should improve authenticity");
...
assert(grayState.partyRuntime.player.reputation.integrity <
    baseState.partyRuntime.player.reputation.integrity, "Gray action should damage integrity");
...
for (const action of blackOps) {
    ...
    assert((action.effects.reputationShift?.integrity ?? 0) < 0 ||
        (action.effects.scandalRiskShift ?? 0) > 0.15, `${action.id} must damage integrity or raise scandal risk`);
