...
function controversyPenaltyForCluster(outlet, trust, speakerRole, format, preparationLevel) {
    ...
    return Math.max(0, controversy - 0.34) * Math.max(0.05, 0.62 - trust) * exports.mediaFormatProfiles[format].riskMultiplier * prepRelief + speakerRisk * Math.max(0, 0.55 - trust);
}
