...
exports.computeElectionResult = computeElectionResult;
exports.estimateSeats = estimateSeats;
exports.coalitionScore = coalitionScore;
exports.answerQuestion = answerQuestion;
exports.acceptSponsor = acceptSponsor;
...
exports.respondToMediaInvitation = respondToMediaInvitation;
exports.receiveMediaInvitations = receiveMediaInvitations;
exports.respondToMediaAppearance = respondToMediaAppearance;
exports.computePolls = computePolls;
exports.computePublicRegionalPolls = computePublicRegionalPolls;
exports.formatPercent = formatPercent;
exports.segmentShareInRegion = segmentShareInRegion;
...
const issueEngine_1 = require("./issueEngine");
const baselineAbstain = 0.18;
const latentDimensions8D = [
    'econ',
    'culture',
    'authority',
    'establishment',
...
function hasComputedSupport(state) {
    const nationalSupportReady = seed_1.partyIds.every((partyId) => Number.isFinite(state.nationalSupport?.[partyId]));
    const regionalSupportReady = state.regions.length > 0 &&
        state.regions.every((region) => seed_1.partyIds.every((partyId) => Number.isFinite(state.regionalSupport?.[region.id]?.[partyId])));
    return nationalSupportReady && regionalSupportReady;
}
function previewActionImpact(state, plannedAction) {
    const actionV2Preview = (0, actionEngine_1.previewCampaignActionV2)(state, plannedAction);
    return {
        delta: 0,
...
function computeElectionResult(state) {
    ...
    const topSeatCount = Math.max(...seed_1.partyIds.map((partyId) => seats[partyId]));
    return {
        nationalSupport: computed.nationalSupport,
        seats,
        winners: seed_1.partyIds.filter((partyId) => seats[partyId] === topSeatCount),
    };
}
...
function coalitionScore(state, coalition) {
...
function answerQuestion(state, questionId, optionId) {
    ...
    nextState.partyRuntime.player.reputation.consistency = clamp(nextState.partyRuntime.player.reputation.consistency + option.consistencyValue * 0.03, 0, 1);
    nextState.partyRuntime.player.reputation.controversy = clamp(nextState.partyRuntime.player.reputation.controversy + option.controversyRisk * 0.08, 0, 1);
    ...
    if (player && option.issueOwnershipShift) {
        for (const [issue, value] of Object.entries(option.issueOwnershipShift)) {
            player.issueOwnership[issue] = clamp((player.issueOwnership[issue] ?? 0) + value, 0, 1);
        }
    }
    return nextState;
}
function acceptSponsor(state, sponsorId) {
    const nextState = cloneState(state);
...
function hireMarketingAdvisor(state, advisorId) {
    const nextState = cloneState(state);
    ...
    nextState.partyRuntime.player.informationQuality = clamp(nextState.partyRuntime.player.informationQuality + advisor.predictionAccuracyBonus * 0.15, 0, 1);
    if (advisor.reputationRisk) {
        nextState.partyRuntime.player.reputation.controversy = clamp(nextState.partyRuntime.player.reputation.controversy + advisor.reputationRisk, 0, 1);
    }
    return initializeComputedState(nextState);
}
function publishPublicPoll(state, pollsterId) {
    const nextState = cloneState(state);
...
function updateProgramIssue(state, issueId, patch) {
    const nextState = cloneState(state);
    ensureIssueLayer(nextState);
...
function respondToMediaInvitation(state, invitationId, response) {
...
function receiveMediaInvitations(state, invitations) {
    const nextState = cloneState(state);
...
function applyMediaAppearanceCosts(state, decision, invitation) {
    ...
    const speakerTime = decision.action === 'decline'
        ? 0.03
        : decision.speakerRole === 'leader'
            ...
            : decision.speakerRole === 'regionalFigure'
                ? 0.14
                : 0.22;
    const fatigue = decision.action === 'decline'
        ? 0.01
...
function computePolls(state, support) {
    ...
    const partySupportEstimate = Object.fromEntries(seed_1.partyIds.map((partyId) => {
    ...
    return {
        ...
        recommendedActions: advisor.level >= 2
            ? [
...
function computePublicRegionalPolls(state, pollsterId) {
    ...
    return Object.fromEntries(state.regions.map((region, regionIndex) => {
        ...
        const estimates = Object.fromEntries(seed_1.partyIds.map((partyId, partyIndex) => {
...
function getRegionalVoterPoints(precision = 'weekly') {
    if (precision === 'full') {
        if (!cachedFullRegionalVoterPoints) {
            ...
            const { loadRegionalizedVoterFieldV03 } = require('../simulation/model/voterFieldLoader');
    ...
    if (!cachedWeeklyRegionalVoterPoints) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const { loadClusteredRegionalizedVoterFieldV03 } = require('../simulation/model/voterFieldLoader');
...
function computeRegionalSupportFromParticles(state, precision, options = {}) {
    ...
    const partyContexts = seed_1.partyIds
        ...
        return { party, partyId, runtime };
    })
        .filter((context) => Boolean(context));
    const totals = Object.fromEntries(state.regions.map((region) => [region.id, Object.fromEntries(seed_1.partyIds.map((partyId) => [partyId, 0]))]));
    ...
    for (const point of compactPoints) {
        ...
        const turnout = (0, actionEngine_1.preparedTurnoutProbability)(point.turnoutBase, region.turnoutModifier);
...
function computeParticleUtilityForContext(state, context, region, point, options = {}) {
    ...
    const reputationFit = 0.18 * reputation.trust +
...
function mediaClusterUtilityModifier(state, point) {
    ...
    return modifiers.reduce((sum, modifier) => {
        const cluster = mediaOutlets_1.voterClusters.find((item) => item.id === modifier.clusterId);
        if (!cluster) {
            return sum;
        }
...
function clusterAffinity(point, cluster) {
    const ideologyDistance = Math.abs(point.position.econ - cluster.ideologyMean.econ) +
...
function demographicAffinity(value, target) {
    if (!value || !target || target === 'mixed')
        return 0.55;
    if (target === 'large_city')
        return value === 'metro' || value === 'large_town' ? 1 : 0.2;
    if (target === 'small_town')
        return value === 'town' || value === 'large_town' ? 1 : 0.25;
    if (target === 'rural')
        return value === 'rural' ? 1 : 0.25;
    if (target === 'high')
        return value === 'tertiary' ? 1 : 0.35;
    if (target === 'medium')
        return value === 'secondary' ? 1 : 0.55;
    if (target === 'low_medium')
        return value === 'lower' || value === 'secondary' ? 0.9 : 0.35;
    if (target === 'medium_high')
        return value === 'secondary' || value === 'tertiary' ? 0.9 : 0.35;
    if (target === '18-29')
        return value === '15_24' || value === '25_39' ? 0.9 : 0.2;
    if (target === '30-49')
        return value === '25_39' || value === '40_54' ? 0.85 : 0.3;
    if (target === '30-59')
        return value === '25_39' || value === '40_54' || value === '55_plus' ? 0.75 : 0.3;
    if (target === '30-69')
        return value === '25_39' || value === '40_54' || value === '55_plus' ? 0.75 : 0.3;
    if (target === '45+' || target === '60+')
        return value === '55_plus' || value === '40_54' ? 0.85 : 0.25;
...
function ensureIssueLayer(state) {
    if (!state.issueLayer) {
...
function applyIssueLayerToPlayer(state) {
    ...
    const derived = (0, issueEngine_1.deriveLatentFromIssues)(state.issueLayer.player.currentIssuePositions, state.issueLayer.issues, state.issueLayer.framings);
    const nextLatent = (0, issueEngine_1.applyIdeologicalInertia)(runtime.field.center8D ?? legacyFieldCenter8D(runtime.field), derived, runtime.field.flexibility);
    ...
    runtime.reputation.consistency = clamp(runtime.reputation.consistency + (state.issueLayer.player.coherenceBreakdown.coherenceScore - 62) / 5000, 0, 1);
}
function applyAction(state, plannedAction, actionEffects, riskNotes) {
    const actionV2Result = (0, actionEngine_1.applyCampaignActionV2)(state, plannedAction);
...
function applyEvents(state, events, actionEffects, riskNotes) {
...
function applyOpponentMove(state, partyId, move, region, rngSeed, opponentMoves, riskNotes) {
    const runtime = state.partyRuntime[partyId];
    const party = state.parties.find((item) => item.id === partyId);
...
function chooseOpponentMove(state, partyId, regionId, roll) {
    const runtime = state.partyRuntime[partyId];
    const party = state.parties.find((item) => item.id === partyId);
...
function selectOpponentTargetRegion(state, partyId, rngSeed) {
...
function opponentMoveCost(move) {
    switch (move) {
        case 'attackPlayer':
            return 0.8;
        case 'leaderTour':
...
function spendOpponentBudget(state, partyId, amount) {
    const runtime = state.partyRuntime[partyId];
...
function selectOpponentIssue(party, seed) {
    const issues = Array.from(new Set([...Object.keys(party.issueOwnership), ...Object.keys(party.issuePositions)]));
    return issues[Math.floor(randomFromSeed(seed) * issues.length) % Math.max(1, issues.length)] ?? 'housing';
}
function widenFieldForIssue(field, issue, amount) {
    if (issue === 'security') {
...
function applyScandals(state, riskNotes) {
...
function applyMarketingAdvisorCost(state, riskNotes) {
    const runtime = state.partyRuntime.player;
    const advisor = currentMarketingAdvisor(state);
    if (advisor.id === 'none') {
        return;
...
function createBriefing(beforeState, nextState, notes) {
...
function getRegionHighlights(before, after) {
...
function getSegmentNotes(state) {
...
function createAdvisorRecommendations(state) {
...
function resetLeaderWeek(state) {
...
function currentMarketingAdvisor(state) {
    ...
    return (state.marketingAdvisors?.find((advisor) => advisor.id === advisorId) ??
...
function increaseRecentConflict(state, partyA, partyB, amount) {
    const relation = findRelation(state, partyA, partyB);
    if (!relation) {
        return;
    }
    relation.recentConflict = clamp(relation.recentConflict + amount, 0, 1);
    relation.totalCoalitionPotential = clamp(relation.publicAcceptability + relation.ideologicalCompatibility + relation.personalTrust - relation.recentConflict - relation.scandalBarrier, 0, 1);
}
function findRelation(state, partyA, partyB) {
    return state.coalitionRelations.find((relation) => (relation.partyA === partyA && relation.partyB === partyB) ||
        (relation.partyA === partyB && relation.partyB === partyA));
}
function labelParty(state, partyId) {
...
function ensurePartyField8D(field, party) {
...
function inferPartyCenter8D(party, fieldRuntime) {
    ...
    return completeLatentVector({
        ...
        econ: field.econ,
        establishment: clamp(latent?.establishment ?? (party.reputation.trust + party.reputation.competence + party.reputation.integrity) / 1.55 - 0.95 - party.reputation.controversy * 0.35, -1, 1),
...
function applyParliamentAttendance(state, riskNotes) {
    ...
    const pressure = (0, programMandateCatalog_1.deriveParliamentAttendancePressure)(availableLeaderHours, availableStaffHours);
...
function legacyFieldCenter8D(field) {
    return completeLatentVector({
...
function legacyFieldWidth8D(field) {
    return completeLatentVector({
...
function defaultFieldSalience8D() {
    return completeLatentVector({
        authority: 1,
        culture: 1,
        econ: 1,
...
function completeLatentVector(vector) {
    return {
...
function widenField8D(field, dimension, amount) {
    field.width8D = field.width8D ?? legacyFieldWidth8D(field);
    field.width8D[dimension] = clamp(field.width8D[dimension] + amount, 0.25, 1.4);
}
function normalizePartyRecord(record) {
    const total = seed_1.partyIds.reduce((sum, partyId) => sum + Math.max(0, record[partyId]), 0);
    const divisor = total || 1;
    return Object.fromEntries(seed_1.partyIds.map((partyId) => [partyId, Math.max(0, record[partyId]) / divisor]));
}
function shiftReputation(reputation, shift) {
...
function segmentShareInRegion(state, regionId, segmentId) {
