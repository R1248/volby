...
const issueEngine_1 = require("../src/game/issueEngine");
const issueSeed_1 = require("../src/game/issueSeed");
const layer = (0, issueSeed_1.createIssueLayerState)();
const programUi = (0, node_fs_1.readFileSync)('app/program.tsx', 'utf8');
const store = (0, node_fs_1.readFileSync)('src/store/useGameStore.ts', 'utf8');
const engine = (0, node_fs_1.readFileSync)('src/game/engine.ts', 'utf8');
const issueEngine = (0, node_fs_1.readFileSync)('src/game/issueEngine.ts', 'utf8');
...
const aaRelation = layer.relations.find((relation) => relation.from === 'affirmativeAction' && relation.to === 'sameSexMarriage');
assertOk(Boolean(aaRelation), 'Missing affirmativeAction -> sameSexMarriage relation');
const forwardPenalty = (0, issueEngine_1.implicationViolation)(positions, aaRelation);
...
const reversePenalty = (0, issueEngine_1.implicationViolation)(reversePositions, aaRelation);
...
const incoherent = (0, issueEngine_1.calculatePartyCoherence)(incoherentParty, layer.issues, layer.relations, layer.ideologicalFrames, 0.6, layer.framings);
const coherent = (0, issueEngine_1.calculatePartyCoherence)(coherentParty, layer.issues, layer.relations, layer.ideologicalFrames, 0.6, layer.framings);
...
const overloadedAgenda = (0, issueEngine_1.calculateAgendaPenalty)(overloaded, layer.issues, []);
...
const agendaWithoutGraph = (0, issueEngine_1.calculateAgendaPenalty)(sameFamilyAgenda, layer.issues, []);
const agendaWithGraph = (0, issueEngine_1.calculateAgendaPenalty)(sameFamilyAgenda, layer.issues, layer.relations);
...
const mobilizingGraph = (0, issueEngine_1.evaluateIssueGraph)(mobilizingPositions, layer.relations, layer.framings);
...
const unframedGraph = (0, issueEngine_1.evaluateIssueGraph)(unframedEnergy, layer.relations, layer.framings);
const framedGraph = (0, issueEngine_1.evaluateIssueGraph)(framedEnergy, layer.relations, layer.framings);
assertOk(framedGraph.unresolvedTensionPenalty < unframedGraph.unresolvedTensionPenalty, 'Useful framing should reduce requires_framing/tension penalty');
...
const irrelevantFramingGraph = (0, issueEngine_1.evaluateIssueGraph)(irrelevantFramingEnergy, layer.relations, layer.framings);
assertOk(Math.abs(irrelevantFramingGraph.unresolvedTensionPenalty - unframedGraph.unresolvedTensionPenalty) < 0.000001, 'Irrelevant framing should not reduce requires_framing/tension penalty');
...
const broadCoherentPenalty = (0, issueEngine_1.calculateAgendaPenalty)(broadCoherentAgenda, layer.issues, layer.relations);
...
const supportiveLayer = (0, issueEngine_1.recalculateIssueLayer)({
    ...layer,
    player: {
        ...layer.player,
...
const opposedLayer = (0, issueEngine_1.recalculateIssueLayer)({
    ...layer,
    player: {
        ...layer.player,
        currentIssuePositions: {
...
assertOk((0, issueEngine_1.issueLayerUtilityModifier)(supportiveLayer, greenVoter, true) > (0, issueEngine_1.issueLayerUtilityModifier)(opposedLayer, greenVoter, true), 'Good issue fit should improve voter utility');
...
assertOk((0, issueEngine_1.issueLayerUtilityModifier)(artificiallyIncoherent, greenVoter, true) < (0, issueEngine_1.issueLayerUtilityModifier)(artificiallyCoherent, greenVoter, true), 'Incoherence should lower voter utility');
...
assertOk((0, issueEngine_1.issueLayerUtilityModifier)(noBonusUtilityLayer, greenVoter, true) === (0, issueEngine_1.issueLayerUtilityModifier)(supportiveLayer, greenVoter, true), 'Graph synergy bonuses should not directly boost voter utility');
...
for (let index = 0; index < layer.mediaQuestions.length; index += 1) {
    ...
    mediaQuestionLayer = (0, issueEngine_1.answerProgramMediaQuestion)(mediaQuestionLayer, pendingId, question?.answerOptions[0]?.id ?? '', 0.6);
}
...
for (let index = 0; index < layer.tripEvents.length; index += 1) {
    ...
    tripLayer = (0, issueEngine_1.answerCampaignTrip)(tripLayer, pendingId, trip?.options[0]?.id ?? '', 0.6);
}
assertOk(tripLayer.pendingCampaignTripId === undefined, 'Campaign trips should not cycle indefinitely');
const latent = (0, issueEngine_1.deriveLatentFromIssues)(layer.player.currentIssuePositions, layer.issues, layer.framings);
...
const coherence = (0, issueEngine_1.calculatePartyCoherence)(layer.player, layer.issues, layer.relations, layer.ideologicalFrames, 0.6, layer.framings);
