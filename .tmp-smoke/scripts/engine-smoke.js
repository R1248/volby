...
const mediaOutlets_1 = require("../src/data/mediaOutlets");
const mediaMiniGameQuestions_1 = require("../src/data/mediaMiniGameQuestions");
const mediaEngine_1 = require("../src/game/mediaEngine");
...
function assertHealthyState(state) {
    ...
    const nationalSum = seed_1.partyIds.reduce((total, partyId) => total + state.nationalSupport[partyId], 0);
...
const baseSeats = (0, engine_1.estimateSeats)(baseState.nationalSupport);
assert(baseSeats.others === 0, 'Others must never receive seats');
const sanity = (0, regionalSanityCheck_1.regionalSanityScore)(baseState.regionalSupport);
...
const biasedRegionalSupport = (0, engine_1.computeRegionalSupport)(baseState, {
    regionalBaselineBias,
    regionalBaselineBiasStrength: 1,
});
const biasedNationalSupport = (0, engine_1.computeNationalSupport)(baseState, biasedRegionalSupport);
const biasedNationalSum = seed_1.partyIds.reduce((sum, partyId) => sum + biasedNationalSupport[partyId], 0);
...
for (const region of baseState.regions) {
    const biasedRegionalSum = seed_1.partyIds.reduce((sum, partyId) => sum + biasedRegionalSupport[region.id][partyId], 0);
...
const issueEdit2 = (0, engine_1.updateProgramIssue)(issueShifted, 'taxes', { salience: 3 });
const issueEdit3 = (0, engine_1.updateProgramIssue)(issueEdit2, 'pensions', { salience: 3 });
const issueEditBlocked = (0, engine_1.updateProgramIssue)(issueEdit3, 'migration', { salience: 4 });
...
const mediaPending = (0, engine_1.respondToMediaAppearance)(baseState, {
    action: 'accept',
...
assert(pendingMediaResult.sentimentRating !== undefined && pendingMediaResult.sentimentRating >= 1 && pendingMediaResult.sentimentRating <= 5, 'Pending media result should expose sentimentRating 1-5');
...
assert(mediaPending.issueLayer.player.currentIssuePositions[mediaInvitation.issueId ?? 'greenDeal']?.salience === mediaBeforeSalience, 'Media response must not immediately apply issue salience');
const mediaResolved = (0, engine_1.resolveTurn)(mediaPending, [], 99).state;
assert((mediaResolved.pendingMediaEffects ?? []).length === 0, 'resolveTurn should clear pending media effects');
assert(mediaResolved.mediaAppearanceResults?.some((result) => result.invitationId === mediaInvitation.id && result.status === 'applied'), 'resolveTurn should mark pending media effect as applied in results');
assert((mediaResolved.mediaClusterModifiers ?? []).length > (baseState.mediaClusterModifiers ?? []).length ||
    mediaResolved.partyRuntime.player.reputation.trust !== mediaBeforeReputation.trust ||
    mediaResolved.partyRuntime.player.momentum !== mediaBeforeMomentum, 'resolveTurn should apply pending media effects before support recomputation');
assert(!(0, node_fs_1.readFileSync)('src/components/media/MediaInvitationCard.tsx', 'utf8').includes('supportDelta'), 'Immediate media UI must not show exact support deltas');
const mediaCardSource = (0, node_fs_1.readFileSync)('src/components/media/MediaInvitationCard.tsx', 'utf8');
...
assert(mediaCardSource.includes('ChipGroup') && mediaCardSource.includes('Problem') && mediaCardSource.includes('Hodnota') && mediaCardSource.includes('Reseni'), 'soundbite_builder should use real chip UI');
...
const publicDebateOutlet = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'public_tv_main_debate');
const toxicOutlet = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'anti_establishment_channel');
const foreignPropagandaOutlet = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'foreign_propaganda_channel');
const progressiveOutlet = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'progressive_activist_magazine');
const regionalOutlet = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'regional_radio');
...
const declineState = (0, engine_1.initializeComputedState)({
    ...baseState,
    mediaInvitations: [highReachDebate, lowReachRegional, toxicPodcast],
});
const highDecline = (0, engine_1.respondToMediaAppearance)(declineState, { action: 'decline', invitationId: highReachDebate.id, preparationLevel: 'none' });
const lowDecline = (0, engine_1.respondToMediaAppearance)(declineState, { action: 'decline', invitationId: lowReachRegional.id, preparationLevel: 'none' });
const toxicDecline = (0, engine_1.respondToMediaAppearance)(declineState, { action: 'decline', invitationId: toxicPodcast.id, preparationLevel: 'none' });
...
assert(Math.abs(highDeclineResult.partyMomentumDelta) > Math.abs(lowDeclineResult?.partyMomentumDelta ?? 0), 'High-reach debate decline should carry stronger pending penalty than low-reach decline');
assert((toxicDeclineResult?.partyMomentumDelta ?? -1) >= (lowDeclineResult?.partyMomentumDelta ?? 0), 'Toxic outlet decline should be neutral or defensible');
const antiSovereigntyResult = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: 'anti-sovereignty-impact', preparationLevel: 'basic', speakerRole: 'controversialFigure' }, (0, engine_1.initializeComputedState)({
    ...baseState,
    mediaInvitations: [testInvitation('anti-sovereignty-impact', toxicOutlet.id, 'nationalSovereignty', 'podcast', 0.35, 0.86, null)],
}));
const antiHealthcareResult = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: 'anti-healthcare-impact', preparationLevel: 'basic', speakerRole: 'controversialFigure' }, (0, engine_1.initializeComputedState)({
    ...baseState,
...
const antiHealthcareInAudience = (antiHealthcareResult.clusterImpacts.find((impact) => impact.clusterId === 'anti_establishment_online')?.impact ?? 0) +
...
const spdAntiFit = (0, mediaEngine_1.calculatePartyOutletFit)(spdLikeState, toxicOutlet, 'player');
const pirateAntiFit = (0, mediaEngine_1.calculatePartyOutletFit)(pirateLikeState, toxicOutlet, 'player');
const spdPublicFit = (0, mediaEngine_1.calculatePartyOutletFit)(spdLikeState, publicDebateOutlet, 'player');
const spdProgressiveFit = (0, mediaEngine_1.calculatePartyOutletFit)(spdLikeState, progressiveOutlet, 'player');
const pirateProgressiveFit = (0, mediaEngine_1.calculatePartyOutletFit)(pirateLikeState, progressiveOutlet, 'player');
const pirateForeignFit = (0, mediaEngine_1.calculatePartyOutletFit)(pirateLikeState, foreignPropagandaOutlet, 'player');
const spdForeignFit = (0, mediaEngine_1.calculatePartyOutletFit)(spdLikeState, foreignPropagandaOutlet, 'player');
...
const spdAntiPendingState = (0, engine_1.initializeComputedState)({ ...spdLikeState, mediaInvitations: [spdAntiInvitation] });
const spdAntiPending = (0, engine_1.respondToMediaAppearance)(spdAntiPendingState, {
    action: 'accept',
...
const spdAntiResolved = (0, engine_1.resolveTurn)(spdAntiPending, [], 321).state;
assert(!spdAntiResolved.scandals.some((scandal) => scandal.id.includes(spdAntiInvitation.id)), 'SPD-like party in PL-like outlet should not automatically create a media scandal from controversy alone');
const spdPublicInvitation = testInvitation('spd-public-fit', publicDebateOutlet.id, 'taxes', 'debate', 0.82, 0.62, null);
const spdPublicPending = (0, engine_1.respondToMediaAppearance)((0, engine_1.initializeComputedState)({ ...spdLikeState, mediaInvitations: [spdPublicInvitation] }), {
    action: 'accept',
...
const spdProgressivePending = (0, engine_1.respondToMediaAppearance)((0, engine_1.initializeComputedState)({ ...spdLikeState, mediaInvitations: [spdProgressiveInvitation] }), {
    action: 'accept',
...
const spdProgressiveResolved = (0, engine_1.resolveTurn)(spdProgressivePending, [], 322).state;
assert((spdProgressiveResolved.mediaClusterModifiers ?? []).some((modifier) => ['anti_establishment_online', 'working_class_protest', 'rural_conservatives'].includes(modifier.clusterId) && modifier.amount < 0), 'Alarm-like mismatch should create negative delayed modifiers among nationalist/anti-system base clusters');
assert(!spdProgressiveResolved.scandals.some((scandal) => scandal.id.includes(spdProgressiveInvitation.id)), 'Low-toxicity Alarm-like mismatch should not necessarily create a global scandal');
...
const pirateProgressiveResult = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: pirateProgressiveInvitation.id, preparationLevel: 'basic', speakerRole: 'leader' }, (0, engine_1.initializeComputedState)({ ...pirateLikeState, mediaInvitations: [pirateProgressiveInvitation] }));
...
const pirateAntiResult = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: pirateAntiInvitation.id, preparationLevel: 'basic', speakerRole: 'leader' }, (0, engine_1.initializeComputedState)({ ...pirateLikeState, mediaInvitations: [pirateAntiInvitation] }));
...
const foreignResult = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: foreignInvitation.id, preparationLevel: 'basic', speakerRole: 'leader' }, (0, engine_1.initializeComputedState)({ ...spdLikeState, mediaInvitations: [foreignInvitation] }));
...
const foreignResolved = (0, engine_1.resolveTurn)((0, engine_1.respondToMediaAppearance)((0, engine_1.initializeComputedState)({ ...spdLikeState, mediaInvitations: [foreignInvitation] }), {
    action: 'accept',
    invitationId: foreignInvitation.id,
    preparationLevel: 'basic',
    speakerRole: 'leader',
}), [], 323).state;
assert(foreignResolved.scandals.some((scandal) => scandal.id.includes(foreignInvitation.id)), 'Sputnik-like toxic outlet should create high media scandal risk after delayed application');
const hostileQuestions = (0, mediaEngine_1.selectMediaMiniGameQuestions)(highReachDebate, publicDebateOutlet, declineState);
...
assert(hostileQuestions.every((question) => question.topicId === 'taxes' || question.isGenericFallback), 'Taxes invitation must not receive unrelated off-topic fallback questions');
assert(mediaMiniGameQuestions_1.mediaMiniGameQuestions.some((question) => question.topicId === 'taxes' && question.questionKind === 'budget_constraint'), 'Tax questions should include a budget_constraint probe');
assert(mediaMiniGameQuestions_1.mediaMiniGameQuestions.some((question) => question.topicId === 'taxes' && question.questionKind === 'coherence_trap'), 'Tax questions should include a coherence_trap probe');
const taxFundingQuestion = mediaMiniGameQuestions_1.mediaMiniGameQuestions.find((question) => question.id === 'taxes-funding-source');
const taxBeneficiaryQuestion = mediaMiniGameQuestions_1.mediaMiniGameQuestions.find((question) => question.id === 'taxes-beneficiaries-first');
const taxCoherenceQuestion = mediaMiniGameQuestions_1.mediaMiniGameQuestions.find((question) => question.id === 'taxes-service-promise-trap');
const taxPositionQuestion = mediaMiniGameQuestions_1.mediaMiniGameQuestions.find((question) => question.id === 'taxes-position-direct');
...
const lowTaxProgramState = (0, engine_1.updateProgramIssue)(baseState, 'taxes', { position: 2, salience: 4 });
const taxFactQuestion = mediaMiniGameQuestions_1.mediaMiniGameQuestions.find((question) => question.id === 'taxes-fact-deficit-debt');
...
const correctFactScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([correctFactAnswer], [taxFactQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'expert',
    state: declineState,
});
const wrongFactScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([wrongFactAnswer], [taxFactQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: lowTaxProgramState,
});
const honestFactScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([honestFactAnswer], [taxFactQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: lowTaxProgramState,
...
const fundedTaxScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([fundedTaxAnswer], [taxFundingQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'expert',
    state: declineState,
});
const unfundedTaxScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([unfundedTaxAnswer], [taxBeneficiaryQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: declineState,
});
assert((unfundedTaxScore.fiscalCredibilityScore ?? 0) < (fundedTaxScore.fiscalCredibilityScore ?? 0) &&
    (unfundedTaxScore.competenceAdjustment ?? 0) < (fundedTaxScore.competenceAdjustment ?? 0), 'Tax cuts for everyone without funding should score worse on fiscal credibility and competence than a funded answer');
const lightOutlet = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'youth_creator_stream');
assert(lightOutlet, 'Expected youth creator outlet');
const vagueFiscalSerious = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([taxFundingQuestion.options.find((answer) => answer.id === 'growth-pays') ?? taxFundingQuestion.options[0]], [taxFundingQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: declineState,
});
const vagueFiscalLight = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([taxFundingQuestion.options.find((answer) => answer.id === 'growth-pays') ?? taxFundingQuestion.options[0]], [taxFundingQuestion], {
...
const coherentTaxScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([coherentTaxAnswer], [taxCoherenceQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'expert',
    state: declineState,
});
const populistTaxScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([populistTaxAnswer], [taxCoherenceQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: declineState,
...
const lowTaxAlignedScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([lowTaxAnswer], [taxPositionQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: lowTaxProgramState,
});
const highTaxMismatchScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([highRevenueAnswer], [taxPositionQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: lowTaxProgramState,
...
const redistributiveProgramState = (0, engine_1.updateProgramIssue)((0, engine_1.updateProgramIssue)(baseState, 'taxes', { position: -2, salience: 4 }), 'redistribution', { position: 2, salience: 3 });
const lowTaxMismatchLeftScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([lowTaxAnswer], [taxPositionQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: redistributiveProgramState,
});
const highTaxAlignedLeftScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([highRevenueAnswer], [taxPositionQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: redistributiveProgramState,
...
const soundbiteLowTaxFit = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([soundbiteLowTaxSolution], [taxPositionQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: lowTaxProgramState,
});
const soundbiteLowTaxMismatch = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([soundbiteLowTaxSolution], [taxPositionQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: redistributiveProgramState,
...
const taxMismatchPending = (0, engine_1.respondToMediaAppearance)((0, engine_1.initializeComputedState)({
    ...lowTaxProgramState,
...
assert(taxMismatchPending.issueLayer.player.currentIssuePositions.taxes.position === lowTaxProgramState.issueLayer.player.currentIssuePositions.taxes.position, 'Tax program commitment effects must not apply immediately');
const taxMismatchResolved = (0, engine_1.resolveTurn)(taxMismatchPending, [], 114).state;
assert(taxMismatchResolved.issueLayer.player.currentIssuePositions.taxes.position < lowTaxProgramState.issueLayer.player.currentIssuePositions.taxes.position, 'High-tax media commitment should shift taxes position downward after resolveTurn');
const taxProgramConsistencyQuestion = mediaMiniGameQuestions_1.mediaMiniGameQuestions.find((question) => question.id === 'taxes-program-lower-and-spend');
...
const taxProgramAlignedScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([taxProgramAlignedAnswer], [taxProgramConsistencyQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: lowTaxProgramState,
});
const taxProgramContradictScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([taxProgramContradictAnswer], [taxProgramConsistencyQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: lowTaxProgramState,
});
const taxProgramEvasiveScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([taxProgramEvasiveAnswer], [taxProgramConsistencyQuestion], {
    invitation: lowTaxProgramDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: lowTaxProgramState,
...
const fiscallyWeakScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([growthPaysAnswer], [taxFundingQuestion], {
    invitation: highReachDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: declineState,
});
const fiscalReputationState = (0, engine_1.initializeComputedState)({
    ...baseState,
...
const fiscalPending = (0, engine_1.respondToMediaAppearance)(fiscalReputationState, {
    action: 'accept',
...
const fiscalResolved = (0, engine_1.resolveTurn)(fiscalPending, [], 115).state;
...
const goodSentimentResult = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: highReachDebate.id, miniGameResult: fundedTaxScore, preparationLevel: 'basic', speakerRole: 'expert' }, declineState);
const badSentimentResult = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: highReachDebate.id, miniGameResult: populistTaxScore, preparationLevel: 'basic', speakerRole: 'expert' }, declineState);
...
assert(((0, mediaEngine_1.mediaSentimentFromResult)(badSentimentResult).rating ?? 5) < 4, 'High-reach debate with bad answers can score below 4/5 sentiment');
const mediumOutletForSentiment = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'business_podcast');
...
const excellentMediumScore = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([correctFactAnswer, fundedTaxAnswer], [taxFactQuestion, taxFundingQuestion], {
    invitation: mediumSentimentInvitation,
    outlet: mediumOutletForSentiment,
    speakerRole: 'expert',
    state: declineState,
});
const excellentMediumResult = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: mediumSentimentInvitation.id, miniGameResult: excellentMediumScore, preparationLevel: 'strong', speakerRole: 'expert' }, (0, engine_1.initializeComputedState)({
    ...baseState,
    mediaInvitations: [mediumSentimentInvitation],
}));
assert(((0, mediaEngine_1.mediaSentimentFromResult)(excellentMediumResult).rating ?? 0) > 3, 'Medium outlet with excellent answers can score above 3/5 sentiment');
for (const templateTopic of new Set(mediaOutlets_1.mediaInvitationTemplates.map((template) => template.topicId))) {
    const topicQuestions = mediaMiniGameQuestions_1.mediaMiniGameQuestions.filter((question) => question.topicId === templateTopic && !question.isGenericFallback);
...
const seriousNewspaper = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'serious_newspaper_interview');
assert(seriousNewspaper, 'Expected serious newspaper outlet');
const genericOnlyQuestions = (0, mediaEngine_1.selectMediaMiniGameQuestions)(genericOnlyInvitation, seriousNewspaper, declineState);
assert(genericOnlyQuestions.every((question) => question.topicId === 'civilServiceReform' && question.isGenericFallback), 'When topic-specific questions are missing, selector should return generic fallback only');
...
assert(genericOnlyQuestions.every((question) => question.prompt.includes(civilServiceLabel)), 'Generic fallback prompt should be adapted to the invitation topic');
assert(genericOnlyQuestions.every((question) => !question.prompt.includes('taxes')), 'Generic fallback tax placeholder must not leak into non-tax invitations');
const longFormInvitation = testInvitation('long-form', 'business_podcast', 'taxes', 'podcast', 0.31, 0.32, 'long_form');
const businessPodcast = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'business_podcast');
assert(businessPodcast, 'Expected business podcast outlet');
const businessTaxAffinity = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: 'business-tax-affinity', preparationLevel: 'basic', speakerRole: 'expert' }, (0, engine_1.initializeComputedState)({
    ...baseState,
    mediaInvitations: [testInvitation('business-tax-affinity', businessPodcast.id, 'taxes', 'podcast', 0.31, 0.32, null)],
}));
const businessUnrelatedAffinity = (0, mediaEngine_1.resolveMediaAppearance)({ action: 'accept', invitationId: 'business-unrelated-affinity', preparationLevel: 'basic', speakerRole: 'expert' }, (0, engine_1.initializeComputedState)({
    ...baseState,
    mediaInvitations: [testInvitation('business-unrelated-affinity', businessPodcast.id, 'sameSexMarriage', 'podcast', 0.31, 0.32, null)],
}));
const totalPositiveImpact = (result) => result.clusterImpacts.reduce((sum, impact) => sum + Math.max(0, impact.impact), 0);
...
const longFormQuestions = (0, mediaEngine_1.selectMediaMiniGameQuestions)(longFormInvitation, businessPodcast, declineState);
...
const commercialOutlet = mediaOutlets_1.mediaOutlets.find((outlet) => outlet.id === 'commercial_tv_evening_show');
assert(commercialOutlet, 'Expected commercial TV outlet');
const soundbiteQuestions = (0, mediaEngine_1.selectMediaMiniGameQuestions)(soundbiteInvitation, commercialOutlet, declineState);
...
const strongMiniGame = (0, mediaEngine_1.scoreMediaMiniGameAnswers)(hostileQuestions.map((question) => question.options[0]), hostileQuestions, { invitation: highReachDebate, outlet: publicDebateOutlet, speakerRole: 'leader', state: declineState });
const weakMiniGame = (0, mediaEngine_1.scoreMediaMiniGameAnswers)(hostileQuestions.map((question) => question.options.find((answer) => answer.tone === 'vague') ?? question.options[0]), hostileQuestions, { invitation: highReachDebate, outlet: publicDebateOutlet, speakerRole: 'leader', state: declineState });
...
const greenPositionQuestion = mediaMiniGameQuestions_1.mediaMiniGameQuestions.find((question) => question.id === 'green-position-direct');
...
const greenAlignedState = (0, engine_1.updateProgramIssue)(baseState, 'greenDeal', { position: 2, salience: 4 });
const greenContradictState = (0, engine_1.updateProgramIssue)(baseState, 'greenDeal', { position: 2, salience: 4 });
const greenDebate = testInvitation('green-commitment', publicDebateOutlet.id, 'greenDeal', 'debate', 0.82, 0.62, 'three_questions_timed');
const alignedProgramResult = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([greenSupportAnswer], [greenPositionQuestion], {
    invitation: greenDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: greenAlignedState,
});
const mismatchProgramResult = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([greenRejectAnswer], [greenPositionQuestion], {
    invitation: greenDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: greenContradictState,
});
assert((alignedProgramResult.programMismatchPenalty ?? 0) < 0.01, 'Aligned position answer should not create meaningful program mismatch penalty');
assert((mismatchProgramResult.programMismatchPenalty ?? 0) > (alignedProgramResult.programMismatchPenalty ?? 0), 'Contradicting position answer should create programMismatchPenalty');
assert((mismatchProgramResult.impliedProgramEffects ?? []).some((effect) => effect.issueId === 'greenDeal' && (effect.consistencyPenalty ?? 0) > 0), 'Strong contradiction in high-reach debate should create pending program effect or consistency penalty');
const commitmentState = (0, engine_1.initializeComputedState)({
    ...greenContradictState,
    mediaInvitations: [greenDebate],
});
const commitmentPending = (0, engine_1.respondToMediaAppearance)(commitmentState, {
    action: 'accept',
...
assert(!commitmentPendingResult.sentimentSummary?.includes(commitmentPendingResult.programWarning.text), 'Program warning should not be merged into sentimentSummary');
assert(commitmentPending.issueLayer.player.currentIssuePositions.greenDeal.position === commitmentState.issueLayer.player.currentIssuePositions.greenDeal.position, 'Pending program effects must not apply immediately');
...
const commitmentResolved = (0, engine_1.resolveTurn)(commitmentPending, [], 112).state;
const positionShift = Math.abs(commitmentResolved.issueLayer.player.currentIssuePositions.greenDeal.position - commitmentState.issueLayer.player.currentIssuePositions.greenDeal.position);
...
const decimalSalienceBase = (0, engine_1.updateProgramIssue)(baseState, 'greenDeal', { position: 2, salience: 1 });
...
const decimalSalienceMiniGame = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([greenRejectAnswer], [greenPositionQuestion], {
    invitation: decimalSalienceDebate,
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: decimalSalienceBase,
});
const decimalSaliencePending = (0, engine_1.respondToMediaAppearance)((0, engine_1.initializeComputedState)({
    ...decimalSalienceBase,
...
const decimalSalienceResolved = (0, engine_1.resolveTurn)(decimalSaliencePending, [], 113).state;
...
const marriageQuestion = mediaMiniGameQuestions_1.mediaMiniGameQuestions.find((question) => question.id === 'marriage-position-direct');
...
const pivotResult = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([marriagePivot], [marriageQuestion], {
    invitation: testInvitation('marriage-pivot', publicDebateOutlet.id, 'sameSexMarriage', 'debate', 0.82, 0.62, 'three_questions_timed'),
    outlet: publicDebateOutlet,
    speakerRole: 'leader',
    state: baseState,
});
const explicitResult = (0, mediaEngine_1.scoreMediaMiniGameAnswers)([marriageExplicit], [marriageQuestion], {
...
const miniGamePending = (0, engine_1.respondToMediaAppearance)(declineState, {
    action: 'accept',
    invitationId: highReachDebate.id,
...
const miniGameResolved = (0, engine_1.resolveTurn)(miniGamePending, [], 111).state;
...
const programAnswerA = (0, engine_1.answerProgramMediaQuestion)(baseState, programQuestionId, programQuestion.answerOptions[0].id);
const programAnswerB = (0, engine_1.answerProgramMediaQuestion)(programAnswerA, programQuestionId, programQuestion.answerOptions[0].id);
assert(JSON.stringify(programAnswerB.issueLayer.player.currentIssuePositions) === JSON.stringify(programAnswerA.issueLayer.player.currentIssuePositions), 'Program media questions cannot be answered repeatedly for effects');
...
const tripAnswerA = (0, engine_1.answerCampaignTrip)(baseState, tripId, trip.options[0].id);
const tripAnswerB = (0, engine_1.answerCampaignTrip)(tripAnswerA, tripId, trip.options[0].id);
assert(JSON.stringify(tripAnswerB.issueLayer.player.currentIssuePositions) === JSON.stringify(tripAnswerA.issueLayer.player.currentIssuePositions), 'Campaign trip events cannot be farmed repeatedly');
...
const debateAnswerA = (0, engine_1.answerDebateAttack)(debateState, 'test-response');
const debateAnswerB = (0, engine_1.answerDebateAttack)({
    ...debateAnswerA,
    issueLayer: {
...
assert(debateAnswerB.issueLayer.player.currentIssuePositions.greenDeal.salience === debateAnswerA.issueLayer.player.currentIssuePositions.greenDeal.salience, 'Debate responses cannot be farmed repeatedly');
...
function stateWithPlayerCenter(state, center8D) {
    return (0, engine_1.initializeComputedState)({
        ...state,
        partyRuntime: {
            ...state.partyRuntime,
            player: {
...
function testInvitation(id, outletId, issueId, format, expectedReach, risk, miniGameType) {
    return {
        expectedReach,
        format,
        id,
