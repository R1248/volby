...
exports.resolveCampaignActionV2 = resolveCampaignActionV2;
exports.getCampaignPhase = getCampaignPhase;
exports.getCampaignTimingMultipliers = getCampaignTimingMultipliers;
exports.previewCampaignActionV2 = previewCampaignActionV2;
exports.applyCampaignActionV2 = applyCampaignActionV2;
exports.clampPreparedTurnoutModifier = clampPreparedTurnoutModifier;
exports.preparedTurnoutProbability = preparedTurnoutProbability;
const campaignActionsV2_1 = require("./campaignActionsV2");
const issueEngine_1 = require("./issueEngine");
const issueSeed_1 = require("./issueSeed");
function resolveCampaignActionV2(state, plannedAction) {
    return (state.campaignActionsV2 ?? campaignActionsV2_1.campaignActionsV2).find((action) => action.id === plannedAction.actionV2Id);
}
function getCampaignPhase(currentWeek, finalWeek) {
    if (currentWeek >= finalWeek)
        return 'election_day';
    const weeksToElection = finalWeek - currentWeek;
    if (weeksToElection <= 1)
        return 'final';
    if (weeksToElection <= 4)
        return 'late';
    if (currentWeek / finalWeek < 0.45)
        return 'early';
    return 'mid';
}
function getCampaignTimingMultipliers(actionId, currentWeek, finalWeek) {
    ...
    return {
        ...
        turnout: actionId === 'gotvOperation'
            ? isFinal
                ? 1.45
                : isLate
                    ? 1.18
...
function previewCampaignActionV2(state, plannedAction) {
    const action = resolveCampaignActionV2(state, plannedAction);
    if (!action) {
        return undefined;
    }
    const runtime = state.partyRuntime.player;
    const precisionPenalty = action.preview.precisionRequired === 'seniorAdvisor'
        ? 0.18
...
function applyCampaignActionV2(state, plannedAction) {
    const action = resolveCampaignActionV2(state, plannedAction);
    const actionEffects = [];
    const riskNotes = [];
    if (!action) {
    ...
    runtime.leader.fatigue = clamp(runtime.leader.fatigue + action.fatigueCost + action.ethicalRisk * 0.04 + action.risks.scandal * 0.03, 0, 1);
    ...
    if (resolvedAction.legality !== 'clean') {
        riskNotes.push(`${resolvedAction.name}: ${resolvedAction.legality === 'illegal' ? 'ilegalni abstraktni mechanika' : 'seda zona'} zvysila expozici, backlash a koalicni toxicitu.`);
    }
    addActionFeedback(state, resolvedAction);
    return { action, actionEffects, handled: true, ok: true, riskNotes };
}
function validateCampaignActionV2(state, action, plannedAction) {
    const runtime = state.partyRuntime.player;
    ensureRuntimeDefaults(state);
...
function applyCampaignTiming(state, action) {
...
function applyFieldEffects(state, action) {
    const runtime = state.partyRuntime.player;
    ...
    for (const [dimension, amount] of Object.entries(action.effects.latentCenterShift ?? {})) {
        field.center8D[dimension] = clamp(field.center8D[dimension] + amount, -1, 1);
    }
    for (const [dimension, amount] of Object.entries(action.effects.latentWidthShift ?? {})) {
...
function applyIssueEffects(state, plannedAction, action) {
    if (!action.effects.issuePositionShift &&
    ...
    const recalculated = (0, issueEngine_1.recalculateIssueLayer)({
        ...state.issueLayer,
        player: {
            ...state.issueLayer.player,
            currentIssuePositions: currentPositions,
            mediaVulnerability: clamp(state.issueLayer.player.mediaVulnerability + (action.effects.mediaVulnerabilityShift ?? 0), 0, 1),
        },
    }, runtime.field.flexibility);
    state.issueLayer = recalculated;
    applyIssueLayerToPlayerField(state);
}
function applyIssueLayerToPlayerField(state) {
    const runtime = state.partyRuntime.player;
    const field = runtime.field;
    field.center8D = field.center8D ?? completeLatentVector(field.latentCenter ?? field.center);
    ...
    const nextLatent = (0, issueEngine_1.applyIdeologicalInertia)(field.center8D, derived, field.flexibility);
    ...
    field.latentCenter = field.center8D;
    runtime.reputation.consistency = clamp(runtime.reputation.consistency + (state.issueLayer.player.coherenceBreakdown.coherenceScore - 62) / 6000, 0, 1);
}
function applyReputationEffects(reputation, shift) {
    if (!shift) {
        return;
    }
    for (const [key, amount] of Object.entries(shift)) {
        reputation[key] = clamp(reputation[key] + amount, 0, 1);
    }
}
function applyOrganizationEffects(state, plannedAction, action) {
...
function applyCoalitionEffects(state, plannedAction, action) {
...
function applyRiskEffects(state, plannedAction, action) {
    const runtime = state.partyRuntime.player;
    runtime.scandalRisk = clamp(runtime.scandalRisk +
        (action.effects.scandalRiskShift ?? 0) +
        action.risks.scandal * 0.025 +
        action.risks.legal * 0.015, 0, 1);
...
function applyTurnoutPreparation(state, action) {
...
function createTurnoutState(state, action, kind, modifier) {
    return {
        actionId: action.id,
...
function addTurnoutModifier(state, action, modifier) {
...
function addActionFeedback(state, action) {
...
function affectedIssues(issuePositionShift, issueSalienceShift, framingChange) {
...
function resolveIssueRecord(record, plannedAction) {
    if (!record) {
        return undefined;
    }
    const targetIssueId = plannedAction.targetProgramIssueId ?? plannedAction.targetIssueId ?? 'housing';
    return Object.fromEntries(Object.entries(record).map(([issueId, value]) => [issueId === 'target' ? targetIssueId : issueId, value]));
}
...
function ensureRuntimeDefaults(state) {
    const runtime = state.partyRuntime.player;
...
function clampPreparedTurnoutModifier(value) {
    return clamp(value, -0.08, 0.08);
}
function scaleOptional(value, multiplier) {
...
function preparedTurnoutProbability(baseTurnout, regionalModifier, modifiers = []) {
