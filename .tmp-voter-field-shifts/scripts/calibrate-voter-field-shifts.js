...
const baselineCalibration_1 = require("../src/game/baselineCalibration");
...
function runCalibration(cliOptions) {
    const rawPoints = (0, voterFieldLoader_1.loadClusteredRegionalizedVoterFieldV03)().points;
    let shifts = {};
    const originalState = createNationallyCalibratedState(shifts, rawPoints);
    const originalRegionalSupport = computeRegionalSupportWithShifts(originalState, rawPoints, shifts);
    const originalRegionalSanity = (0, regionalSanityCheck_1.regionalSanityScore)(originalRegionalSupport);
    const originalNationalSupport = computeNationalSupportWithPoints(originalState, originalRegionalSupport, rawPoints);
    const originalNationalGroupSanity = createNationalGroupSanity(originalNationalSupport);
    const iterations = [];
    for (let iteration = 0; iteration <= cliOptions.iterations; iteration += 1) {
        ...
        const regionalSupport = computeRegionalSupportWithShifts(state, rawPoints, shifts);
        const nationalSupport = computeNationalSupportWithPoints(state, regionalSupport, rawPoints);
        const regionalSanity = (0, regionalSanityCheck_1.regionalSanityScore)(regionalSupport);
        const nationalGroupSanity = createNationalGroupSanity(nationalSupport);
        iterations.push({
            iteration,
            nationalGroupMaxErrorPct: Math.max(...nationalGroupSanity.map((row) => row.errorPct)),
            regionalSanity,
            shifts: clone(shifts),
        });
        ...
        const delta = proposeRegionShiftDelta(state, rawPoints, shifts, regionalSupport, cliOptions);
        shifts = mergeShifts(shifts, delta, cliOptions);
    }
    const finalState = createNationallyCalibratedState(shifts, rawPoints);
    const finalRegionalSupport = computeRegionalSupportWithShifts(finalState, rawPoints, shifts);
    const finalNationalSupport = computeNationalSupportWithPoints(finalState, finalRegionalSupport, rawPoints);
    const finalRegionalSanity = (0, regionalSanityCheck_1.regionalSanityScore)(finalRegionalSupport);
    const finalNationalGroupSanity = createNationalGroupSanity(finalNationalSupport);
...
function createRawSeedState() {
    return (0, seed_1.createInitialGameState)();
}
function createAmplitudeOneState() {
...
function createNationallyCalibratedState(shifts, points) {
    const state = (0, seed_1.createInitialGameState)();
    return (0, baselineCalibration_1.calibratePartyAmplitudesToTargets)(state, seed_1.baselineTargetShares, {
        disablePlayerProgramModifier: true,
        supportResolver: (candidateState, supportOptions) => {
            const regionalSupport = computeRegionalSupportWithShifts(candidateState, points, shifts, supportOptions);
            return {
                nationalSupport: computeNationalSupportWithPoints(candidateState, regionalSupport, points),
                regionalSupport,
            };
        },
    });
}
function calibrateStateForShifts(shifts, points) {
    return createNationallyCalibratedState(shifts, points);
}
function evaluateScenario(label, state, points, shifts) {
    const scenarioState = cloneState(state);
    const regionalSupport = computeRegionalSupportWithShifts(scenarioState, points, shifts);
    const nationalSupport = computeNationalSupportWithPoints(scenarioState, regionalSupport, points);
    const nationalGroupSanity = createNationalGroupSanity(nationalSupport);
    return {
        label,
        nationalGroupMaePct: average(nationalGroupSanity.map((row) => row.errorPct)),
        nationalGroupMaxErrorPct: Math.max(...nationalGroupSanity.map((row) => row.errorPct)),
        nationalGroupSanity,
        nationalSupport,
...
function createScenarioComparison(points, finalShifts) {
    const rawSeedState = createRawSeedState();
    const amplitudeOneState = createAmplitudeOneState();
    const nationallyCalibratedRawState = createNationallyCalibratedState({}, points);
    const nationallyCalibratedShiftedState = createNationallyCalibratedState(finalShifts, points);
    return [
        evaluateScenario("raw seed + raw field", rawSeedState, points, {}),
        evaluateScenario("raw seed + proposed shifts", rawSeedState, points, finalShifts),
        evaluateScenario("amplitude=1 + raw field", amplitudeOneState, points, {}),
        evaluateScenario("amplitude=1 + proposed shifts", amplitudeOneState, points, finalShifts),
        evaluateScenario("nationally calibrated + raw field", nationallyCalibratedRawState, points, {}),
        evaluateScenario("nationally calibrated + proposed shifts", nationallyCalibratedShiftedState, points, finalShifts),
    ];
}
function computeRegionalSupportWithShifts(state, points, shifts, options = {}) {
    const totals = Object.fromEntries(state.regions.map((region) => [
        region.id,
        Object.fromEntries(seed_1.partyIds.map((partyId) => [partyId, 0])),
    ]));
...
function computeUtilityForPoint(input) {
    ...
    const logUtility = Math.log(Math.max(0.04, input.runtimeField.amplitude)) +
...
function proposeRegionShiftDelta(state, points, currentShifts, regionalSupport, cliOptions) {
    ...
    for (const [regionId, target] of Object.entries(regionalVoteTargets2025_1.regionalVoteTargets2025)) {
        ...
        const modeledGroups = (0, regionalSanityCheck_1.aggregateForCalibration)(modeled);
...
function mergeShifts(current, delta, cliOptions) {
    const merged = {};
    for (const regionId of Object.keys(regionalVoteTargets2025_1.regionalVoteTargets2025)) {
        ...
        for (const dimension of dimensions) {
            const raw = currentShift[dimension] * cliOptions.regularization +
                deltaShift[dimension];
            next[dimension] = clamp(raw, -cliOptions.clampShift, cliOptions.clampShift);
        }
...
function calibrationGroupCenter(state, groupId) {
    ...
    const totalTarget = partyIdsForGroup.reduce((sum, partyId) => sum + (seed_1.baselineTargetShares[partyId] ?? 0), 0) || 1;
    ...
    for (const partyId of partyIdsForGroup) {
        ...
        const weight = (seed_1.baselineTargetShares[partyId] ?? 0) / totalTarget;
        for (const dimension of dimensions) {
...
function parseOptions(args) {
    return {
        ...
        outputPath: stringArg(args, "--output", "src/game/calibration/proposed-region-latent-shifts.v04.json"),
...
function printReport(report, cliOptions) {
    ...
    console.log(`options: iterations=${cliOptions.iterations}, learningRate=${cliOptions.learningRate}, regularization=${cliOptions.regularization}, clampShift=${cliOptions.clampShift}`);
    console.log("\nOriginal regional sanity");
    console.log(`MAE ${formatPp(report.originalRegionalSanity.maePct)} | max ${formatPp(report.originalRegionalSanity.maxErrorPct)}`);
    console.log("\nIteration sweep");
    for (const iteration of report.iterations) {
        console.log(`iter ${iteration.iteration}: regional MAE ${formatPp(iteration.regionalSanity.maePct)}, max ${formatPp(iteration.regionalSanity.maxErrorPct)}, national max error ${formatPp(iteration.nationalGroupMaxErrorPct)}`);
    }
    console.log("\nFinal regional sanity");
    console.log(`MAE ${formatPp(report.finalRegionalSanity.maePct)} | max ${formatPp(report.finalRegionalSanity.maxErrorPct)}`);
    ...
    for (const { regionId, shift, norm } of Object.entries(report.finalShifts)
        ...
        .slice(0, 14)) {
        console.log(`${regionId}: norm ${norm.toFixed(3)} | ${dimensions
...
function printScenarioComparison(scenarios) {
...
function printScenarioNationalErrors(scenarios, labels) {
...
function printScenarioRegionalErrors(scenarios, labels) {
    for (const label of labels) {
        const scenario = requireScenario(scenarios, label);
...
function requireScenario(scenarios, label) {
