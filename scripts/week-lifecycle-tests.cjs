...
Module._load = function (request, parent, isMain) {
  if (request === '@/src/game/storage') return {
    loadLatestGame: async () => saved && copy(saved),
    saveGame: async (state, plannedActions) => { saved = copy({ state, plannedActions }); saves++; },
    resetSave: async () => { saved = undefined; },
    saveTurnSnapshot: async () => { snapshots++; },
  };
...
const { prepareWeek, resolvePreparedWeek, resolveTurn, initializeComputedState, respondToMediaAppearance } = require('../src/game/engine.ts');
const { createInitialGameState } = require('../src/game/seed.ts');
const { generateWeeklyMediaInvitations } = require('../src/game/mediaEngine.ts');
...
async function main() {
  ...
  let seed = 1;
  while (((Math.sin(seed + legacy.week * 17) * 10000) % 1 + 1) % 1 >= 0.08) seed++;
  legacy.rngSeed = seed;
  ...
  assert.deepEqual(prepared.partyRuntime, legacy.partyRuntime, 'preparation preserves resources');
  ...
  assert.throws(() => prepareWeek({ ...prepared, week: prepared.week + 1 }), /must be prepared/);
  assert.throws(() => resolvePreparedWeek({ ...prepared, mediaInvitations: [] }, []), /missing context/);
  const reloaded = copy({ schemaVersion: 1, state: prepared, plannedActions: [] }).state;
  ...
  assert.equal(result.state.rngSeed, (seed * 1664525 + 1013904223) % 4294967296);
  ...
  assert.deepEqual(result.state.mediaInvitations.map(x => x.id), prepared.mediaInvitations.map(x => x.id));
  assert.equal(result.state.mediaInvitations.filter(x => x.week === result.state.week).length, 0);
  assert.equal(result.state.scandals.filter(x => x.id === `sponsor-risk-${legacy.week}`).length, 1);
  const next = prepareWeek(result.state);
  assert.deepEqual(next.preparedWeek.invitationIds, generateWeeklyMediaInvitations(result.state).map(x => x.id));
  assert.equal(new Set(next.mediaInvitations.map(x => x.id)).size, next.mediaInvitations.length);
  assert.equal(prepareWeek(next), next);
  const eventState = copy(legacy);
  const event = { ...eventState.events[0], id: 'prepared-event', title: 'Prepared event', week: eventState.week, opportunityFor: ['player'] };
  ...
  withLateEvent.events.push({ ...event, id: 'late-event', title: 'Late event' });
  const eventResult = resolvePreparedWeek(withLateEvent, []);
  assert(eventResult.briefing.actionEffects.some(note => note.includes('Prepared event')));
  assert(!eventResult.briefing.actionEffects.some(note => note.includes('Late event')));
  assert.deepEqual(eventResult.briefing.events, withEvent.preparedWeek.contextNotes);
  assert.throws(() => resolvePreparedWeek({ ...withEvent, events: [] }, []), /missing context/);
  const invitation = prepared.mediaInvitations[0];
  const answered = respondToMediaAppearance(prepared, { action: 'decline', invitationId: invitation.id, preparationLevel: 'none' });
  ...
  assert(answeredResult.state.mediaAppearanceResults.some(x => x.invitationId === invitation.id && x.status === 'applied'));
  ...
  assert.deepEqual(changedResult.briefing.events, prepared.preparedWeek.contextNotes);
  assert.deepEqual(changedResult.state.mediaInvitations.map(x => x.id), prepared.preparedWeek.invitationIds);
  ...
  assert.deepEqual(prepareWeek(oldSave).mediaInvitations, copy(oldSave.mediaInvitations));
  ...
  assert.equal(saves, savesBefore, 'repeated preparation must not persist again');
  ...
  assert.deepEqual(store.getState().gameState.preparedWeek, persisted.state.preparedWeek);
  assert.deepEqual(store.getState().gameState.mediaInvitations, persisted.state.mediaInvitations);
  assert.deepEqual(store.getState().gameState.scandals, persisted.state.scandals);
  saved = { state: oldSave, plannedActions: [{ id: 'legacy-plan', actionV2Id: 'videoClipCampaign' }] };
  await store.getState().hydrateGame();
  assert(store.getState().gameState.preparedWeek);
  assert.equal(store.getState().plannedActions[0].id, 'legacy-plan');

  const planningBase = copy(first);
  const action = planningBase.campaignActionsV2.find(x => x.target.scope === 'region' && x.cost > 0 && x.staffCost > 0 && x.leaderTimeCost > 0 && x.legality === 'clean');
  assert(action, 'need a representative constrained action');
  const cases = [
    ['cash', s => { s.partyRuntime.player.cash = action.cost - 1; }],
    ['legal spending', s => { s.partyRuntime.player.legalSpend = s.rules.legalSpendCap - action.cost + 1; }],
    ['overall spending', s => { s.partyRuntime.player.graySpend = s.rules.spendCap - action.cost + 1; }],
    ['staff', s => { s.partyRuntime.player.staffUsed = s.partyRuntime.player.staffCap - action.staffCost + 0.1; }],
    ['leader time', s => { s.partyRuntime.player.leader.timeUsed = s.partyRuntime.player.leader.timeCap - action.leaderTimeCost + 0.1; }],
    ['required target', () => {}],
  ];
  for (const [name, patch] of cases) {
    ...
    assert.equal(store.getState().planCampaignActionV2(action.id, name === 'required target' ? undefined : 'praha'), false, name);
  ...
  assert(store.getState().gameState.preparedWeek, 'planning prepares the current week');
  assert.equal(store.getState().planCampaignActionV2(action.id, 'praha'), false, 'reserved cash counts');
  store.getState().removePlannedAction(store.getState().plannedActions[0].id);
  assert.equal(store.getState().planCampaignActionV2(action.id, 'praha'), true);
  console.log('Week lifecycle and store tests passed (including all six planning constraints).');
}
