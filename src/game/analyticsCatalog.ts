...
export type AnalyticsActionTemplate = ActionTemplate & {
  placement: 'analytics';
...
export const analyticsActionTemplates: AnalyticsActionTemplate[] = analytics.map(([id, name, description, effects], index) => ({
  availability: 'player_initiated',
  placement: 'analytics',
  id,
  name,
  ...
  cost: actionCost({ money: 0.22 + index * 0.035, centralStaffHours: 5 + (index % 4), fatigue: 0.004 }),
  effects,
  risks: actionRisks({ mediaBacklash: 0.01, messageDiscipline: 0.01 }),
  tags: ['analytics', 'no_direct_support'],
})) as AnalyticsActionTemplate[];
