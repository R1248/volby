...
import {
  ...
  type CampaignActionTemplate,
} from "./campaignActionCatalog";
...
export const campaignActionsV2: CampaignActionV2[] =
  campaignActionTemplates.map(toCampaignActionV2);
...
function toCampaignActionV2(
  ...
  return {
    ...
    staffCost: round(
      (template.cost.centralStaffHours + template.cost.regionalStaffHours) / 40,
    ),
    leaderTimeCost: round(template.cost.leaderHours / 40),
    fatigueCost: round(template.cost.fatigue),
    cooldownWeeks:
      template.cost.durationWeeks > 1 ? template.cost.durationWeeks : undefined,
    target: {
      scope: targetScope,
...
function pickTargetScope(
  ...
  const preferred = (
    ["national", "opponent", "issue", "region", "segment"] as ActionTarget[]
  ).find((target) => template.allowedTargets.includes(target));
...
function mapEffects(
  ...
  if (risks.counterMobilization > 0.14) {
    effects.counterMobilizationRiskShift = round(
      risks.counterMobilization * 0.02,
    );
  }
  ...
  if (risks.scandal > 0.08) {
    effects.scandalRiskShift = round(
      risks.scandal * (template.legality === "illegal" ? 0.16 : 0.08),
    );
  }

  if (risks.legal > 0.08) {
    effects.legalExposureShift = round(
      risks.legal * (template.legality === "illegal" ? 0.18 : 0.08),
    );
  }
...
function mapRisks(template: CampaignActionTemplate): CampaignActionV2["risks"] {
  ...
  return {
    backlash: clamp(
      Math.max(
        risks.mediaBacklash,
        risks.longTermTrust * 0.8,
        risks.counterMobilization * 0.55,
      ),
      0,
      1,
    ),
    ...
    media: clamp(
      Math.max(
        ...
        mediaBase,
        risks.detection * 0.6,
        risks.messageDiscipline,
      ),
      0,
      1,
    ),
    scandal: clamp(
      Math.max(risks.scandal, scandalBase, risks.detection * 0.8),
      0,
      1,
    ),
    counterMobilization: clamp(risks.counterMobilization, 0, 1),
    internalFaction: clamp(
      Math.max(risks.internalFaction, risks.messageDiscipline * 0.6),
      0,
      1,
    ),
...
function filterLatent(record: Record<string, number> | undefined) {
  ...
  const dimensions = new Set<LatentDimension8D>([
    "authority",
    "culture",
    "econ",
    "establishment",
...
function precisionFor(
  template: CampaignActionTemplate,
): CampaignActionV2["preview"]["precisionRequired"] {
  if (template.legality !== "clean") return "seniorAdvisor";
  if (
    template.tags.includes("personal_contact") ||
    template.tags.includes("mobilization")
  )
    return "none";
  if (
    template.tags.includes("advertising") ||
...
function riskLabel(
  legality: CampaignActionV2["legality"],
  risks: CampaignActionV2["risks"],
) {
