import type { DimensionId, GreenDealAttitude, VoterPoint } from './types';
...
export const GREEN_DEAL_PROPENSITY_WEIGHTS = {
  culture: -0.25,
  econ: -0.05,
  establishment: 0.2,
  globalism: 0.3,
...
export const GREEN_DEAL_ATTITUDE_TARGETS = {
  mixed: 0.12,
  opportunity: 0.28,
  threat: 0.6,
} as const;

export type GreenDealThresholds = {
  threatMax: number;
  opportunityMin: number;
};

export function calculateGreenDealPropensity(point: Pick<VoterPoint, 'legacy' | 'position'>) {
  const calculatedFrom7D =
    GREEN_DEAL_PROPENSITY_WEIGHTS.green * point.position.green +
    GREEN_DEAL_PROPENSITY_WEIGHTS.globalism * point.position.globalism +
    GREEN_DEAL_PROPENSITY_WEIGHTS.establishment * point.position.establishment +
    GREEN_DEAL_PROPENSITY_WEIGHTS.ukraine * point.position.ukraine +
    GREEN_DEAL_PROPENSITY_WEIGHTS.culture * point.position.culture +
    GREEN_DEAL_PROPENSITY_WEIGHTS.econ * point.position.econ;
  const legacy = point.legacy?.greenDealPropensityRaw;

  return legacy === undefined
    ? calculatedFrom7D
    : (1 - GREEN_DEAL_PROPENSITY_WEIGHTS.legacyGreenDeal) * calculatedFrom7D +
        GREEN_DEAL_PROPENSITY_WEIGHTS.legacyGreenDeal * legacy;
}

export function calculateGreenDealThresholds(points: readonly VoterPoint[]): GreenDealThresholds {
  ...
  return {
    opportunityMin: weightedQuantile(values, GREEN_DEAL_ATTITUDE_TARGETS.threat + GREEN_DEAL_ATTITUDE_TARGETS.mixed),
    threatMax: weightedQuantile(values, GREEN_DEAL_ATTITUDE_TARGETS.threat),
  };
}

export function classifyGreenDealAttitude(propensity: number, thresholds: GreenDealThresholds): GreenDealAttitude {
...
export function greenDealIssuePosition(propensity: number, thresholds: GreenDealThresholds) {
...
export function enrichGreenDealIssuePreferences(points: VoterPoint[]): VoterPoint[] {
...
export function greenDealWeightedDistribution(points: readonly VoterPoint[]) {
  const totals: Record<GreenDealAttitude, number> = { mixed: 0, opportunity: 0, threat: 0 };
...
export function weightedCorrelation(points: readonly VoterPoint[], dimension: DimensionId) {
  ...
  const meanY =
    points.reduce((sum, point) => sum + point.weight * (point.issuePreferences?.greenDeal ?? 0), 0) / totalWeight;
  let cov = 0;
  let varX = 0;
  let varY = 0;
...
function weightedQuantile(values: { value: number; weight: number }[], quantile: number) {
...
function scaleWithinBand(value: number, min: number, max: number, targetMin: number, targetMax: number) {
